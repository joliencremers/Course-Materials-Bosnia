...
dat <- read.table(
  ...
  , sep=",")
```
Instead, we can read as character, clean up the text and convert to numeric. Or we could go the tidyverse way or use the `read_csv` function in the `readr` package. 
...
### 4 - a walkthrough for the `editrules` package 
...
iris.correction.rules <- correctionRules( "if( Petal.Width <= 0 ){ Petal.Width <- NA } ", 
                                          file = FALSE )

# We use the correction rules on the data
dirty.iris <- correctWithRules( iris.correction.rules , dirty.iris)
```
